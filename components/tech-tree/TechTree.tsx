...
export default function TechTree({ title, nodes }: TechTreeProps) {
  // overall bounds (so the scroll area always fits)
  const CANVAS_W = 2600
  const CANVAS_H = 1200

  // ZOOM OUT so you can see top+bottom without vertical scrolling
  const SCALE = 0.72
  ...
  return (
    <div className="rounded-2xl bg-gradient-to-b from-[#0b4e87] to-[#08345d] p-4 shadow-2xl">
      ...
      <div className="relative w-full h-[520px] overflow-x-auto overflow-y-hidden">
        {/* scaled wrapper: height shrinks, so all rows fit */}
        <div
              ...
              return (
                <div
                  ...
                  className={clsx(
                    ...
                    'hover:shadow-[0_0_70px_rgba(0,200,255,0.95)]',
                    'hover:scale-[1.05]',
